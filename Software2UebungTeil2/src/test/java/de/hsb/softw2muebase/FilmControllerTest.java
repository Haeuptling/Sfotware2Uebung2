...
//Testet, ob bei einem neu erstellten Film Status Created zurück gegeben wird
    @ParameterizedTest
    @ValueSource(strings={"Matrix","","Matrix Reloaded", "Matrix Revolutions"})
    public void returnContainsInput(String input){
        Film newFilm = new Film(input);
        filmController.newMovie(newFilm);
>>>>>>> origin/master
...
=======
    ...
    //Testet, ob all() das gesamte repository wiedergibt
    @ParameterizedTest
    @ValueSource (strings = {"Interstellar", "Scream"})
    void testAllMethode(String input) throws Exception {
        Film newFilm = new Film(input);
        filmController.newMovie(newFilm);


        List<Film> outputFilmList = new ArrayList<>();
        outputFilmList.add(new Film(input));

        Assert.assertNotEquals(outputFilmList, filmController.all());
>>>>>>> origin/master
    }
